...
char shellcode_begin[], shellcode_end[];
...
int kload()
{
    ...
    // get app mainBundle .app/pwnediBSS
    const char *file_pwnediBSS = "/pwnediBSS";
    char *pwnediBSS;
    pwnediBSS = malloc(strlen(app_bundle) + strlen(file_pwnediBSS) + 1);
    strcpy(pwnediBSS, app_bundle);
    strcat(pwnediBSS, file_pwnediBSS);
    printf("pwnediBSS: %s\n", pwnediBSS);
    
    // chown root:wheel
    //chown(exec_kloader, getuid(), getgid());
    chown(pwnediBSS, getuid(), getgid());
    // chmod 755 rwx r-x r-x
    //chmod(exec_kloader, S_ISUID|S_IRUSR|S_IRWXU|S_IRGRP|S_IXGRP|S_IROTH|S_IXOTH);
    // chmod 644 rw- r-- r--
    chmod(pwnediBSS, S_ISUID|S_IRUSR|S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH);
